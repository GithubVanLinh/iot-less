...
router.get('/',async function(req, res, next) {
  ...
  res.render('index', { title: 'Express', dataControl });
});


router.post('/',async function(req, res, next){
  const led = req.body.led;
  const dataControl = await ControlModel.updateOne({_id: "5fa7dee2d608ce50b8057128"}, {Light: led});
  await UserStatusModel.updateOne({id: "5fa7dee2d608ce50b8057128"}, {Status: 1});
  res.redirect("/");
});
